...
#
# 企図:
#   - 「ゲーム開発やQA事業で使用できるAIツール制作」に直結する実装方針。
#   - 学習（高速）と本番運用（無改造ゲーム操作）を分離し現実的なワークフローを示す。
#   - 既存資産に侵襲しないため、現場導入コスト/リスクを最小化。
#
# 使い方:
#   1) 元ゲームを cave_game.py として保存
#   2) pip install "stable-baselines3[extra]==2.3.0" gymnasium pygame
#   3) 学習:
#        python cave_game_reinforcement_learning.py --train --timesteps 300000
#   4) 自動プレイ:
#        python cave_game_reinforcement_learning.py --play --game_module cave_game --seconds 90
# =============================================================================
